...
Having more than one thread in a single process means more independent execution units that can run concurrently at the same time. Threads are small execution units inside a process, and they share some resources as described in the image below.
...
### Thread States
Threads go through a lifecycle that changes their state.

Thread states:
- **New**: A new thread has been created.
- **Ready (Runnable)**: The thread is simply ready for running.
- **Running**: The thread is currently running.
- **Waiting**: The thread is waiting for a specific event to execute.
- **Blocked**: The thread got blocked while running for a specific reason. For example, waiting for an I/O device.
- **Dead/Terminated** : Thread has been terminated.


## Code 
To create and manage threads in C, we will be using the POSIX Threads (pthread) library as follows. 

First, let's create a program without any threads. If there is no thread created, it means there is only one, which is the main thread to execute the program instructions.
...
Now, let's create a new thread (new execution flow) to print another statement, which is `Hello from the new thread`. This can be done by first including the thread library, creating the thread, and then joining the thread with the main execution flow as follows. 
...
`pthread_join()` is a function that will join two threads together; in our case, it will wait for `thread1` to execute and then continue the execution of the main thread. If we changed its position to after the main thread's print message, the output of the program may change, since we don't require thread1 to finish before printing `Hello from main thread`.
...
You can also print the thread ID using the variable we created as follows. 
...
There is a shortcut to implement an interface in Java without the effort to create a class, as follows.
```java
public class CreateThread2 {
...
> [!NOTE]
> `sleep()` is called as `Thread.sleep(3000)` rather than `t0.sleep(3000)`. This is because `sleep()` is a **static** method on the `Thread` class, it always pauses whichever thread is currently executing the call, regardless of which object it's called on. 
