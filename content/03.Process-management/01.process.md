...
When a program is executed, it is loaded into the main memory as a process to be handled by the operating system, from execution to assigning resources. Once the process has completed its task, the operating system will free the memory addresses and resources from its use.
...
### Process Tree
Every process is part of another process. If a running program (process) executes another program, the first one will be the parent of the second one. Hence, process relationships form a tree data structure.

Let us have an example of running the **date program** from the terminal to display today's date. 
...
> [!NOTE]
> The root process is the first process that the computer runs. In Unix-based OS, it is typically the init process. 
...
> [!NOTE]
> There are some differences between operating systems and the names of their syscalls. For example, creating a process in Windows OS can be achieved using the *CreateProcess()* syscall.
