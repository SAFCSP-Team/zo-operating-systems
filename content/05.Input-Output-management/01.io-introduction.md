...
> [!NOTE]
> The user and kernel space are execution contexts where the kernel has complete privileges and the user has limited privileges to execute.
...
Once the resource has been used and the OS finishes its task, control will be given back to the user program after switching to user mode.
