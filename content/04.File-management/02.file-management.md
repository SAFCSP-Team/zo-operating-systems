...
### Open a file
To open a file, we will use `fopen` function. Note, that our project has two files in the same level (directory). The `open.c` and `file.txt`. To open `file.txt` from our C program, we will have the following code in `open.c`.
```c
#include <stdio.h>
