...
![process image with static and dynamic sections ](./images/10.proces-sections.jpg)
...
![process image](./images/07.process.jpg)
...
There is more information, some of which is specific to the operating system needs, for example, the process priority, to understand when it will be executed.

## Process State Life-Cycle
Each process has the information about its state. The states the process can be in are the following. 
...
![process state transition image ](./images/09.process-state.png)
...
Later on, we will deep dive into how processes are mapped and managed, and how we can request services from the operating system using syscalls.
